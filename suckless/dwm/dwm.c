		...
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				drw_text(drw, x, 0, tabw, bh, lrpad / 2 + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), c->name, 0);
			  	int x_position = x + lrpad / 2;
				int y_position = (bh - m->sel->ich) / 2;
				int width = m->sel->icw;
				int height = m->sel->ich;
				if (c->icon) {
					drw_pic(drw, x_position, y_position, width, height, c->icon);
				} else if (m->sel->icon) {
					drw_pic(drw, x_position, y_position, width, height, m->sel->icon);
				}
				x += tabw;
			}
		} else {
